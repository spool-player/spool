    ...
    const QByteArray& zipBytes, QString *errorMessage)
{
    ...
    for (int i = 0; i < eocd->numEntries; ++i) {
        ...
        if (cd->method == 0) {
            ...
            // Deflated
            unsigned long destLen = cd->uncompressedSize;
            unsigned long srcLen = cd->compressedSize;
            const int ret = puff(reinterpret_cast<unsigned char *>(uncompressed.data()), &destLen, fileData, &srcLen);
            if (ret != 0 || destLen != cd->uncompressedSize) {
        ...
        const uint32_t calculatedCrc
            = calculateCrc32(reinterpret_cast<const uint8_t *>(uncompressed.constData()), uncompressed.size());
        if (calculatedCrc != cd->crc32) {
            setError(errorMessage, QStringLiteral("CRC-32 mismatch for %1").arg(entryPath));
            return std::nullopt;
        }
...
uint32_t ProviderPackage::calculateCrc32(const uint8_t *data, size_t length)
{
    static const uint32_t table[256] = { 0x00000000, 0x77073096, 0xee0e612c, 0x990951ba, 0x076dc419, 0x706af48f,
        ...
        0x0ead5439, 0x9dd264af, 0x04db3515, 0x73dc0583, 0xe3630b12, 0x94643b84, 0x0d6d6a3e, 0x7a6a5aa8, 0xe40ecf0b,
        ...
        0xd8062bca, 0xaf011b5c, 0x360847e6, 0x410f7770, 0xdf60efc3, 0xa867df55, 0x316e8eef, 0x4669be79, 0xcb61b38c,
